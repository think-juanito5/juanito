...
export const prepareStepchangeToSds = async (
  ...
  const smsEmailProcEnabled = await j5config.get(
    J5Config.actionstep.smsEmailProcEnabled,
  )

  await logger.info(
    `prepareStepchangeToSds(+) smsEmailProcEnabled: ${smsEmailProcEnabled?.value} #Action matterId: ${matterId} Change Step Node Id =>`,
  ...
  const sdsStep = stepsArr.find(
    (item) => item.stepName === 'Seller Disclosure Statement',
  )
  ...
  await logger.debug(
    `prepareStepchangeToSds(+) stepMessages|stepTasks =>`,
    stepMessages,
    stepTasks,
  )
  //StepMessages is supported in prod environment only
  const isSmsEmailEnabled =
    ['prod'].includes(appEnv) && smsEmailProcEnabled?.value === 'true'
  const sdsStepMessages: string[] = isSmsEmailEnabled
    ? (stepMessages || []).map((item) => item.split('--')[2])
    : []

  await logger.info(
    `prepareStepchangeToSds(*) isSmsEmailEnabled =>`,
    `${isSmsEmailEnabled} smsEmailProcEnabled: ${smsEmailProcEnabled?.value}`,
  )
  ...
  await logger.debug(
    `prepareStepchangeToSds(-) sdsStepMessages|myStepTasks =>`,
    sdsStepMessages,
    myStepTasks,
  )
