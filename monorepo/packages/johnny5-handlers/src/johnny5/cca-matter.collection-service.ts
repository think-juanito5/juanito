...
export class CcaCollectionHelper {
  ...
  async processMatterCollectionRecords(
    ...
    await this.logger.debug(
      `processMatterCollectionRecords(+) @matterId:${this.matterId} #cRecords length: ${crefRecords?.length}`,
    )
    ...
    if (crefRecords) {
      for (const row of crefRecords) {
        ...
        if (
          ...
          await this.logger.debug(
            `processMatterCollectionRecords(*) @matterId:${this.matterId} #Updated collection record: ${res.datacollectionrecordvalues.id} with value: ${res.datacollectionrecordvalues.stringValue}`,
          )
        }
      }
    }
