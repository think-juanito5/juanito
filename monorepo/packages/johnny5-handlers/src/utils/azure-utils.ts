...
export async function uploadStreamtoAzureStorage(
  blobName: string,
  stream: Readable,
): Promise<Result<BlobUploadCommonResponse, string>> {
  try {
    const fileService = new FileService(factory)
    const response = await fileService.uploadStream(blobName, stream)
    return Ok(response.val)
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error)
    return Err(`error details: ${errorMessage}`)
...
/**
 * Downloads a document from Azure Storage.
 * @param {string} blobName - The name of the blob to download.
 * @returns {Promise<Result<Stream, string>>} - A promise that resolves to an Ok result containing the stream if successful,
 * or an Err result containing an error message if the download fails.
 * @throws {Error} - Throws an error if the download operation encounters an issue.
 */
export const downloadDocumenFromAzureStorage = async (blobName: string) => {
  try {
    const fileService = new FileService(factory)
    const stream = await fileService.download(blobName)
    return stream ? Ok(stream) : Err('stream is not available')
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error)
    return Err(`error details: ${errorMessage}`)
  }
