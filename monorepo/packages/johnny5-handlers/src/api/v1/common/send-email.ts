...
import { jobEmailCloudEventSchema } from '@dbc-tech/johnny5/typebox/email-message.schema.ts'
...
export const send_email = new Elysia()
  .use(jobContext({ name: 'v1.send-email' }))
  ...
  .post(
    ...
    async ({ body, ctx }) => {
      const { logger } = ctx
      const { data } = body
      const { tenant, fileId, jobId, recipients, subject, message } = data
      await logger.info(
        `Starting to send email for File Id:${fileId}, Job Id:${jobId}`,
      )
      ...
      try {
        ...
        await logger.error(
          `Failed to send email for File Id:${fileId}, Job Id:${jobId}`,
          errJson,
        )
        return dapr.drop
      }
      ...
      await logger.info(
        `Finished sending email for File Id:${fileId}, Job Id:${jobId}`,
      )
      return dapr.success
    },
    {
      body: jobEmailCloudEventSchema,
      response: {
