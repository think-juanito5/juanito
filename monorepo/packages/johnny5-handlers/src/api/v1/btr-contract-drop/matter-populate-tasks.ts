...
export const matter_populate_tasks = new Elysia()
  .use(jobContext({ name: 'v1.btr-contract-drop.matter-populate-tasks' }))
  .onRequest(({ request }) => setJsonContentHeader(request))
  .post(
    '/matter-populate-tasks',
    async ({ body, ctx }) => {
      ...
      if (matterData.status === 'tasks') {
        matterData.status = 'files'
        await matterData.save()
        await populator.addTasks(manifest.tasks)
      }
      ...
      await logger.info(
        `Finished populating Matter (Tasks) for File Id:${fileId}, Job Id:${jobId}`,
      )
