...
import { BtrManifestBuilder } from '../../../johnny5/btr/manifest-builder'
import { jobContext } from '../../plugins/job-context.plugin'

export const manifest_create = new Elysia()
  .use(jobContext({ name: 'v1.btr-contract-drop.manifest-create' }))
  ...
  .post(
    ...
    async ({ body, ctx }) => {
      ...
      await logger.info(
        `Starting to create Manifest for File Id:${fileId}, Job Id:${jobId}`,
      )
      await logger.debug('Event payload', body)

      try {
        const builder = new BtrManifestBuilder(
          ctx.actionstep(),
          ctx.logger,
          ctx.correlationId,
        )
