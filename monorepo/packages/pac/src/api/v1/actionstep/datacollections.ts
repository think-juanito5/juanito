import { PagedDataCollectionsSchema } from '@dbc-tech/actionstep'
...
export const datacollections = new Elysia().use(appContext()).post(
  '/getdatacollections',
  async ({
    body,
    ctx,
    ...
    return RemoveNulls(await as.getDataCollections(query))
  },
  {
    body: GetMultipleRequestSchema,
    response: {
      200: transformNullableToOptional(PagedDataCollectionsSchema),
    },
  },
)
