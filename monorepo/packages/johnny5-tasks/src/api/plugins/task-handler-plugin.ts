...
export const runTask = async (args: {
  ...
  const { logger, name, task, file, status } = ctx

  let baseMessage = `Task ${name}, id: ${task.id}, fileId: ${file.id}`
  if (file.pipedriveDealId) {
    baseMessage += `, dealId: ${file.pipedriveDealId}`
  }
  if (file.actionStepMatterId) {
    baseMessage += `, matterId: ${file.actionStepMatterId}`
  }

  await logger.info(`${baseMessage}, started`)

  try {
    const response = (await handler({ ctx, body })) || handlerStatus.success()
    const daprResponse = await getDaprResponse(response, ctx)
    await logger.info(`${baseMessage}, finished`, response)
    return daprResponse
  } catch (error) {
    const jsonError = serializeError(error)
    await logger.error(`${baseMessage}, unhandled error`, jsonError)
