...
export const btrSdsMatterCreatedTeamsSubscribers = async (
  configService: Johnny5ConfigService,
  logger: Logger,
): Promise<TeamsSubscriber[]> => {
  const subscribers: TeamsSubscriber[] = []
  const matterCreatedSubscriber = await getTeamsChannelSubscriber(
    configService,
    J5Config.btr.teams.sdsMatterCreatedMessages.teamsId,
    J5Config.btr.teams.sdsMatterCreatedMessages.channelId,
    ['completed'],
    logger,
  )
  if (matterCreatedSubscriber) {
