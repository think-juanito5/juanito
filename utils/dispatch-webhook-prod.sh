...
if [ -f "${TARGET_BTR_RESPONSE}" ]; then
    curl -X POST -H "Content-Type: application/json" -d @${TARGET_BTR_RESPONSE} https://api.conveyancing.com.au/integrations/v1/typeform/webhook
    test -f ${TARGET_BTR_RESPONSE} && rm ${TARGET_BTR_RESPONSE} > /dev/null
else  
    curl -X POST -H "Content-Type: application/json" -d @./samples/typeform/webhooks/btr-contract-additional-questions.json https://api.conveyancing.com.au/integrations/v1/typeform/webhook
fi
